...
'
' SmashEd
' the particle editor for Smash Particle Engine
'
'
...
tp.Emitter[0].RenderType = TEmitter.RENDER_ALPHA
tp.SetLoop(True)

Global tempE:TEmitter
tempE = New TEmitter
...
Function DoGadgetAction(source:Object, data:Int)
	...
	Select source
		...
		Case RenderType
			Select data
				Case 0
					tp.Emitter[CurrentLayer].RenderType = TEmitter.RENDER_ALPHA
				Case 1
					tp.Emitter[CurrentLayer].RenderType = TEmitter.RENDER_ADD
				Case 2
					tp.Emitter[CurrentLayer].RenderType = TEmitter.RENDER_MUL
				Case 3
					tp.Emitter[CurrentLayer].RenderType = TEmitter.RENDER_OFF
			End Select
	...
	If SelectImageView
		...
		For Local x:Int = 0 To 2
			For Local y:Int = 0 To 2
				...
				DrawImage ParticleSprites,64 + x*128,64 + y*128,count
				count:+1
			Next
		Next
	Else
...
Function ClearLayerValues()			
	...
	tp.Emitter[CurrentLayer] = New TEmitter
	tp.Emitter[CurrentLayer].RenderType = TEmitter.RENDER_OFF
	UpdateGadgetValues()
	UpdateGadgetText()	
End Function

Function CopyObject(src:TEmitter, dst:TEmitter)
	dst.Frame = src.Frame
	dst.X = src.X
	dst.Y = src.Y
